      ...
      <DashboardLayout>
          ...
          <div className={styles.createPostContainer}>
            <img className={styles.userProfile} src={
              authState.user?.userId?.profilePicture?`${BASE_URL}/${authState.user.userId.profilePicture}`:"default.jpg"} alt=""></img>
              <textarea onChange={(e)=>{
                setPostContent(e.target.value)
              }} value={postContent} className={styles.textAreaOfContent} placeholder={"What's on your mind?"}></textarea>
                  ...
                  <div key={post._id} className={styles.singleCard}>
                    <div className={styles.singleCard__profileContainer}>
                      <div className={styles.cardTopLeft}>
                        <img style={{borderRadius:"100%"}} width={60} className={styles.j} src={`${BASE_URL}/${post.userId.profilePicture}`}></img>
                      </div>
                      <div className={styles.del}>
                        ...
                        <svg onClick={async ()=>{
                          ...
                          }} style={{height:"1.3em", color:"red"}} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
                          <path strokeLinecap="round" strokeLinejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                        </svg>
                        </div>
                        }
                        </div>
                        <p style={{color:"gray", fontSize:"0.8rem", marginTop:"-0.2rem"}}>@{post.userId.username}</p>
                        <p>{post.body}</p>
                      </div>
                      </div>
                      ...
                      <div className={styles.optionsContainer}>
                        ...
                        <div className={styles.singleOption__optionsContainer} >
                          <svg onClick={()=>{
                            ...
                            const url = encodeURIComponent("http://localhost:3000/dashboard");
...

        {
          ...
          <div onClick={()=>{
            ...
            <div onClick={(e)=>{e.stopPropagation()}} className={styles.allCommentsContainer}>
                  ...
                  {postState.comments.map((postComment, index)=>{
                    ...
                    <div className={styles.singleComment} key={postComment._id}>
                      <div className={styles.singleComment__profileContainer}>
                        <img width={60} height={60} src={`${BASE_URL}/${postComment.userId.profilePicture}`}></img>
                        <div>
                          <p  style={{fontWeight:"bold"}}>{postComment.userId.name}</p>
                          <p style={{fontSize :"0.7rem"}}>@{postComment.userId.username}</p>
              ...
              <div onClick={(e)=>{e.stopPropagation()}} className={styles.postCommentContainer}>
                <input value={commentText} onChange={(e)=>setCommentText(e.target.value)} placeholder='Comment'></input>
                <div onClick={async()=>{
